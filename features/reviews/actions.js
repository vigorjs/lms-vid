...
export async function saveReview(_state, formData) {
  try {
    ...
    const completedAt = values.outcome === "PASSED" ? publishedAt : null;
    const scoreRows = submission.course.rubricCriteria.map((criterion) => ({
      criterionId: criterion.id,
      criterionTitle: criterion.title,
      weight: criterion.weight,
      score: byId.get(criterion.id),
    }));
    const commentRows = values.comments.map((comment) => ({ ...comment, authorId: actor.id }));
    const reviewValues = {
      reviewerId: actor.id,
      status: "PUBLISHED",
    ...
    await db.$transaction([
      db.review.upsert({
        ...
        create: {
          ...
          comments: { create: commentRows },
        },
        update: {
          ...reviewValues,
          scores: { deleteMany: {}, create: scoreRows },
          comments: { deleteMany: {}, create: commentRows },
        },
      }),
      db.submission.update({ where: { id: submission.id }, data: { status: "REVIEWED" } }),
      db.enrollment.upsert({
        where: { courseId_studentId: { courseId: submission.courseId, studentId: submission.studentId } },
        create: { courseId: submission.courseId, studentId: submission.studentId, completedAt },
        update: { completedAt },
      }),
    ]);
