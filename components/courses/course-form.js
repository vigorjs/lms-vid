...
export function CourseForm({ course, categories, teachers, isAdmin }) {
  ...
  return <form action={formAction} className="grid gap-6"><StateToast state={state} />{course ? <input type="hidden" name="id" value={course.id} /> : null}<input type="hidden" name="criteria" value={JSON.stringify(criteria)} />
    <div className="grid gap-4 md:grid-cols-2"><Field label="Judul course"><input name="title" className={inputClass} defaultValue={course?.title} placeholder="Gaya bebas" required /></Field><Field label="Kategori"><select name="categoryId" className={inputClass} defaultValue={course?.categoryId} required><option value="">Pilih kategori</option>{categories.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></Field></div>
    ...
    <Field label="Deskripsi"><textarea name="description" className={textareaClass} defaultValue={course?.description} placeholder="Tujuan dan ringkasan materi pembelajaran…" required /></Field>
    <div className="grid gap-4"><div className="flex items-center justify-between"><div><h3 className="font-bold text-slate-900">Rubric penilaian</h3><p className="text-xs text-slate-500">Bobot seluruh kriteria harus berjumlah 100%.</p></div><span className={`rounded-full px-3 py-1 text-sm font-bold ${total === 100 ? "bg-emerald-50 text-emerald-700" : "bg-rose-50 text-rose-700"}`}>{total}%</span></div>
      {criteria.map((item, index) => <div key={index} className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-[1fr_1.3fr_100px_40px]"><input aria-label={`Kriteria ${index + 1}`} className={inputClass} value={item.title} onChange={(event) => change(index, "title", event.target.value)} placeholder="Nama kriteria" required /><input aria-label="Deskripsi kriteria" className={inputClass} value={item.description} onChange={(event) => change(index, "description", event.target.value)} placeholder="Penjelasan" /><input aria-label="Bobot" type="number" min="1" max="100" className={inputClass} value={item.weight} onChange={(event) => change(index, "weight", event.target.value)} required /><Button type="button" variant="ghost" className="px-0 text-rose-600" disabled={criteria.length === 1} onClick={() => setCriteria((items) => items.filter((_, i) => i !== index))}><Trash2 size={18} /></Button></div>)}
