...
import { StudentAssignmentPicker } from "@/components/courses/student-assignment-picker";
import { getCourseCoverUrl } from "@/lib/image/url";

export function CourseForm({ course, categories, teachers, students = [], isAdmin }) {
  ...
  const [selectedStudentIds, setSelectedStudentIds] = useState(course?.assignments?.map((assignment) => assignment.studentId) || []);
  const total = useMemo(() => criteria.reduce((sum, item) => sum + Number(item.weight || 0), 0), [criteria]);
  const assignmentRequired = course?.status === "PUBLISHED" && visibility === "ASSIGNED";
  function change(index, key, value) { setCriteria((items) => items.map((item, i) => i === index ? { ...item, [key]: key === "weight" ? Number(value) : value } : item)); }
  return <form action={formAction} className="grid gap-6"><StateToast state={state} />{course ? <input type="hidden" name="id" value={course.id} /> : null}<input type="hidden" name="criteria" value={JSON.stringify(criteria)} /><input type="hidden" name="visibility" value={visibility} /><input type="hidden" name="studentIds" value={JSON.stringify(selectedStudentIds)} />
    ...
    <StudentAssignmentPicker students={students} visibility={visibility} selectedIds={selectedStudentIds} onVisibilityChange={setVisibility} onSelectedIdsChange={setSelectedStudentIds} requireSelection={assignmentRequired} />
