...
import { AdvancedVideoEditor } from "@/components/video/advanced-video-editor";
import { VideoVersionHistory } from "@/components/video/video-version-history";
...
export default async function CourseDetailPage({ params }) {
  ...
  if (user.role === "STUDENT") {
    ...
    submissions = await db.submission.findMany({ where: { courseId, studentId: user.id }, include: { activeVideo: true, originalVideo: true, videoVersions: { where: { status: "READY" }, orderBy: [{ versionNumber: "asc" }, { createdAt: "asc" }] }, review: { include: { scores: true, comments: { orderBy: { timestampSeconds: "asc" } } } } }, orderBy: { attemptNumber: "desc" } });
  }
  ...
  return <><PageHeader eyebrow={`${course.category.name} · ${course.status}`} title={course.title} description={course.description} actions={canManageCourse(user, course) ? <Link href={`/teacher/courses/${course.id}/edit`} className="inline-flex h-11 items-center rounded-xl border border-slate-300 bg-white px-4 text-sm font-semibold">Kelola course</Link> : null} />
    ...
    {canPractice ? <div className="mt-7 grid gap-6 xl:grid-cols-[1.2fr_0.8fr]"><div className="grid content-start gap-6">{active?.status === "DRAFT" ? <><Card><CardHeader><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-cyan-50 text-cyan-700"><Scissors size={20}/></span><div><h2 className="font-bold">Advanced editor attempt {active.attemptNumber}</h2><p className="text-xs text-slate-500">Susun segmen dan transformasi visual; original tetap aman.</p></div></div></CardHeader><CardContent><AdvancedVideoEditor key={active.activeVideoId} courseId={course.id} submissionId={active.id} sourceAssetId={active.activeVideoId} durationSeconds={active.activeVideo.durationSeconds} sourceSizeBytes={active.activeVideo.sizeBytes} /></CardContent></Card><Card><CardContent className="pt-6"><VideoVersionHistory submissionId={active.id} activeVideoId={active.activeVideoId} versions={active.videoVersions}/></CardContent></Card></> : null}{active?.review ? <StudentFeedback review={active.review} threshold={course.passThreshold} /> : null}</div>
